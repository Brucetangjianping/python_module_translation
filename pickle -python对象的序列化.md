...
这就是为什么会限制可序列化的函数和类必须得使用模块中最高等级的定义方式进行定义

类似地，当类的实例被序列化时，类的代码和数据并没有跟着被序列化，只有当实例的数据被序列化的时候，这时才会序列化实例的数据。这是有意这样设计，因此你可以在类中修改bug或者新增函数，而pickler加载的对象仍然是我们最早版本的类。如果你想创建一个长寿的对象来见证类版本的变迁，你可以将版本的数字放入对象中，这样类方法--setstate--()方法会做相应的转换。

## 序列化类的实例
在本节中，我们会给您叙述关于一般机制可用来定义、自定义以及控制类的实例的序列化和反序列化

在大多数情况下，并不需要多余的代码来使得实例可以被序列化，默认情况下，pickle将会通过自省机制（introspection）来检索类和类实例的属性。当一个类的实例被反序列化的时候，不会调用它的--init--（）函数，默认的行为首先是创建一个未初始化的实例，然后存储已保存的属性。以下代码会模拟这个行为：
```
def save(obj):
...
类可以通过一个或一些特殊的方法来改变这些默认的行为：

object.**__getnewargs_ex__()**: <br/>
    在协议4或者更新的协议中，实现了该方法的类可以规定在反序列化的时候将值传给--new--()方法。该方法需要返回一对值（args,kwargs）,args是位置参数的元组，而kwargs是构造对象时的named参数字典，这些值将会在反序列化的时候传递给--new--（）方法
    当--new--()方法需要传递keyword-only参数时，你应该实现该方法，否则，建议实现--getnewargs--()更加兼容的方法
 
 object.**__getnewargs__()**: <br/>
     该方法提供了与--getnewargs_ex--() 类似的功能，但是可以兼容协议2和更新的协议，该方法必须返回args参数的元组,而args是在反序列化时传递给--new--()方法。
     
     在协议4或者更新的协议中，该方法不会被调用，而是使用--getnewargs_ex--()方法当定义了--getnewargs_ex--()方法时
     
object.**__getstate__**():<br/>
    如果一个类中定义了该方法，那么该类可以在实例被序列化的时候可以影响实例的序列化，当调用该函数时，返回对象是当做实例的内容来实例化的，而不是实例的字典内容，如果没有定义该方法，那么实例的--dict--正常实例化。
 
 object.**__setstate__(state)**:<br/>
 当进行反序列化时，如果类定义了该方法，则会带没有序列化状态的参数调用该方法，在这种情况下，则无需将这个状态对象转换成一个字典，否则的话，必须转换成一个字典，而且它的元素将会分配给实例的新创建的字典。

``` 
 注意: 如果--getstate--()返回的是一个false, 那么这个--setstate--()将不会在反序列化的时候调用
 ```
 
 ```
 在反序列的时候，像--getattr--(),--getattribute--()或--setattr--()这类方法，会在这个实例上调用，
 以防这些方法会依赖类内部的不变量进行调用。这种类型需要实现--getnewargs--()或者--getnewargs-ex--()
 来展现不变量。否则，--new--()和--init--()也不会被调用
 ```
 
 我们也将看到，pickle不会直接使用上述的方法。实际上，这些方法只是实现--reduce--()方法时the copy protocol的一部分而已,而the copy protocol为序列化和复制对象时必须检索数据的一个统一的接口。
 尽管很强大，但是在你的类中直接使用--reduce--()方法是一个错误的倾向。因此，类的设计人员应该在有这方面的需求时使用更高级的接口（例如：--getnewargs-ex--(),--getstate--()和--setstate--()），我们将展示使用--reduce--()只是唯一的选择或者可以带来更高效的序列化或者两者都有的情况
