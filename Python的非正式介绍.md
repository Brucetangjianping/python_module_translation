...
[]
```
内建函数`len()`也可以被lists使用
```
>> letters = ['a', 'b', 'c', 'd']
>>> len(letters)
4
```
lists还可以存储lists类型的数据，如下所示：
```
>>> a = ['a', 'b', 'c']
>>> n = [1, 2, 3]
>>> x = [a, n]
...
```
#### 编程的第一步
当然， 我们可以使用python做比两个数相加更复杂的任务， 例如，我们可以像下面一样做一个从1开始的斐波那契的子数组
```
#斐波那契数列： 下一个数是前两个数之和
>>> a, b = 0, 1
>>> while b < 10:
...
```
这个例子说明了几个新的特性：
* 第一行代码给多个变量赋值了，变量a和b分别被赋值了0和1。在最后一行，又使用了这个特性，表明了右手边的表示式在赋值之前就开始执行了，右手边的表达式是从左到右执行的。
* `while`循环在条件(例子是 b < 10)为真时一直执行。 python和C类似，任何非0的整数的值都是真(true)，0则是假(false)。条件也可以是字符串或list的值, 事实上，任何的序列，长度非零则为真，空序列则是假，测试中使用的表达式只是一个简单的比较。 标准的比较操作符和C的写法是一样的，`<` (小于)，`>`(大于),`==`(大于)， `<=`(小于等于)， `>=`(大于等于)和`!=`(不等于)
* 循环体是缩进的，缩进是Python来区分语句的方法，在交互式的命令中，你得在每一个需要缩进的行键入一个tab键或者空格进行缩进。实际上，你需要在文本编辑器进行复杂的输入编辑， 所以比较好的文本编辑器都是有自动缩进功能。当一个复合语句输入完成后，必须要空一行来表明你的语句已经输入完了(如果不空一行的话，编辑器是不知道你的复合语句是否已结束)。注意，在同一个代码块中的行缩进必须是相同的。
* `print()`函数会打印给定的参数值，这和只写表达式(就像我们之前写过的计算例子)的时候，可能有点不同。`print()`可以处理不同类型的参数，浮点数和字符串。打印的字符串内容不会带引号，并且在每个元素之间使用空格隔开。所以你可以将打印格式弄的好看些，就像下面一样：
```
>>> i = 256*256
>>> print('The value of i is', i)
The value of i is 65536
```
关键字end参数可以用来避免输出时换行，或者可以在输出内容后面加入不同的字符串。
```
>>> a, b = 0, 1
>>> while b < 1000:
...     print(b, end=',')
...
```
#### 脚注
[1] 由于`**` 比`-`的优先级更高，`-3**2`将会解析成`-(3**2)`,计算的结果为`-9`,为了避免这种情况，希望得到的结果为`9`,你可以使用`(-3)**2`


[2] 不像其它的语言，特殊的字符像`\n`在单引号(`'...'`)和双引号(`"..."`),这两者的不同点是在单引号中不必转义双引号(`"`)，但是你需要转义单引号(`'`)，反之亦然
